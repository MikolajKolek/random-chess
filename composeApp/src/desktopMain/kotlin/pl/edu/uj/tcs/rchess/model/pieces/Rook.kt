...
class Rook(private var square: Square, private var owner: PlayerColor) : Piece(square = square, owner = owner) {
    override fun getMoveVision(board: BoardState): List<Square> {
        TODO("Implement rook's move vision.")
    }

    override fun getCaptureVision(board: BoardState): List<Square> {
        TODO("Implement rook's capture vision.")
    }
}