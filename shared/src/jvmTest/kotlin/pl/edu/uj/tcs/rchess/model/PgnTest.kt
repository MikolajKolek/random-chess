...
class PgnTest {
    ...
    fun pgnPositionsTest() {
        ...
        pgn = Pgn(
            "[Event \"Eastern SuperBlitz Arena\"]\n" +
    ...
    fun moveSuffixAnnotationsTest() {
        val pgn = Pgn(
            "[Event \"Blitz Titled Arena May '25\"]\n" +
    ...
    fun fenExternalTest() {
        ...
        val fenGeneratorPath = System.getProperty("user.dir") + "/src/jvmTest/fen_generator.py"
        val pgnDatabasePath = System.getProperty("user.dir") + "/src/jvmTest/pgn_database.pgn"
        ...
        for(game in games) {
            ...
            game.metadata?.jsonObject?.filterKeys { it != "Opening" && it != "ECO" && it != "SetUp" }?.forEach { (key, value) ->
    ...
    fun moveExternalTest() {
        ...
        val moveCheckerPath = System.getProperty("user.dir") + "/src/jvmTest/move_checker.py"
        val pgnDatabasePath = System.getProperty("user.dir") + "/src/jvmTest/pgn_database.pgn"
    ...
    fun pgnExportExternalTest() {
        ...
        val games = runBlocking {
            Pgn.Companion.fromPgnDatabase(File(pgnDatabasePath).readText())
        }
        val generator = ProcessBuilder("python3", fenGeneratorPath, pgnDatabasePath).start()
        val reader = BufferedReader(InputStreamReader(generator.inputStream))
        for(importGame in games) {
            val historyGame = PgnGame(
                ...
                opening = Opening("A00", "Placeholder", BoardState.initial, 0),
                startingPosition = importGame.startingPosition,
                //finalPosition = BoardState.initial, // Unused data
                creationDate = OffsetDateTime.now(), // Unused data
                result = importGame.result,
                metadata = Json.decodeFromJsonElement<Map<String, String>>(importGame.metadata!!),
                blackPlayer = PlayerDetails.Simple(importGame.blackPlayerName),
                whitePlayer = PlayerDetails.Simple(importGame.whitePlayerName),
                clockSettings = null,
            )
            var game: Pgn? = null
            try {
